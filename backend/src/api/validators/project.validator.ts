...
export const createProjectValidator = [
  ...
  body('repoUrl').trim().notEmpty().withMessage('Repository URL is required').isURL(),
  body('branch').optional().trim().isLength({ max: 100 }),
  body('projectType').isIn(['NODE_BACKEND', 'REACT_FRONTEND', 'STATIC_SITE', 'FULLSTACK', 'CUSTOM_DOCKERFILE'])
