...
import { GitBranch, Container, Shield, Terminal, Scan, Zap, Code2, FolderTree, LucideIcon } from 'lucide-react';

function Icon3D({ icon: Icon, gradient, shadow }: { icon: LucideIcon; gradient: string; shadow: string }) {
  return (
    <div className="icon-3d mb-5">
      <div className="icon-3d-inner relative inline-block">
        <div
          className={`icon-3d-face relative p-3.5 rounded-2xl bg-gradient-to-br ${gradient}`}
          style={{ '--icon-shadow': shadow } as React.CSSProperties}
        >
          <Icon className="h-6 w-6 text-white relative z-10" strokeWidth={1.8} />
        </div>
        <div className={`icon-3d-bottom bg-gradient-to-r ${gradient}`} />
      </div>
    </div>
  );
}
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {features.map((feature, i) => (
            ...
            >
              <Icon3D icon={feature.icon} gradient={feature.gradient} shadow={feature.shadow} />
